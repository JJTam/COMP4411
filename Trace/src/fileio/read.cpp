	...
	const mmap& materials, TransformNode *transform)
{
	if (name == "translate") {
		...
		if (name == "sphere") {
			...
			ball1pos = tupleToVec(getField(child, "ball1pos"));
			ball2pos = tupleToVec(getField(child, "ball2pos"));
			maybeExtractField(child, "threshold", threshold);

			obj = new Metaball(scene, mat, ball1pos, ball2pos, threshold);
		}
...
static void processObject(Obj *obj, Scene *scene, mmap& materials)
{
	...
	if (name == "directional_light") {
		...
		scene->add(new SpotLight(scene,
			...
			hasField(child, "cone_dir") ? tupleToVec(getField(child, "direction")).normalize() : vec3f(0, 0, 0),
